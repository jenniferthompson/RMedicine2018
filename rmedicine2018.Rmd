...
date: 'R/Medicine 2018 • Yale University<br><br>`r icon::fa("link")` [bit.ly/jlt-rmed2018](https://bit.ly/jlt-rmed2018)<br>`r icon::fa("twitter")` [jent103](https://twitter.com/jent103)'
output:
...
Here I've shown an example file structure with deidentified components for a real study that we have ongoing. This is pretty typical of what I need for my projects; you may need something different, but the point is to find an organized structure that works for you. I find it helpful to have a folder for my analysis datasets; a folder for my R scripts that usually have helper functions that I use in my primary RMarkdown report; and a folder for the publication or presentation versions of figures that I create. But the most important aspect is that one project equals one directory.

When things are organized well, it's not only aesthetically and mentally pleasing, but makes things
...
The tradeoff to this is that you do need to be paying attention: sink those error attributes messages to a text file or take some other tactic, so you don't defeat the purpose and miss a potential problem. But you're in control of how that happens!
