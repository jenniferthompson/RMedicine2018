...
- Facilitate use of [`packrat`](https://cran.r-project.org/package=packrat) and version control
]
...
If you're an RStudio user, using Projects can be helpful here. The first reason I started using them was the ability to have more than one RStudio session open at once with no headaches. But now, I appreciate just as much that they encourage a "one project, one directory" mindset, since each analysis gets its own directory, which gets one project file. Even more helpful is that this is also the structure that works best with version control, and Projects have several features that help facilitate getting git or other version control set up.
...
- First, this is more secure than passing around even password-protected  Excel spreadsheets, since you need an individualized API token to access the database; I usually store my token in my .Renviron file so that I can share the extraction code freely as needed without compromising security.
...
- Next, these lines tell me exactly what data I exported - there's a **record** of exactly what we did. Here, I chose to export only the monthly data collection form, along with the patient ID that's collected on a separate form, and to export only the records related to our monthly data collection. It's clear to anyone coming after me what data I used to perform my next steps.
...
If you're new to working with APIs, the `httr` package has some great vignettes; if you do use REDCap, Benjamin Nutter has written a wrapper package specifically for the REDCap API. If you're using other sources of web data, rOpenSci has a collection of packages for working with APIs that you should investigate.
...
In our example study, we believe that any creatinine above 10 must be some kind of data issue. So we use `assertr`'s `verify` function at the beginning of a data management pipeline to "verify" that that assumption is true. If that condition is met, the function will return the original data.frame, and we can continue with our data management.
